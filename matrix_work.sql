--ПОЛНЫЙ ТЕКСТ ПРОЦЕДУР И ФУНКЦИ В КОНЦЕ ФАЙЛАА
...
-- Процедура обновления товаров Полная
CREATE PROCEDURE update_products()
BEGIN
	TRUNCATE  test_matrix.lost_category;
	TRUNCATE  test_matrix.lost_products;
	TRUNCATE  test_matrix.lost_atr ;
	START TRANSACTION;
	-- Заполним таблицу lost_products
	INSERT INTO test_matrix.lost_products (model)
		SELECT id.model  FROM 
	...
	-- Заполним таблицу lost_atr
	INSERT INTO test_matrix.lost_atr (name,hash )
	SELECT  DISTINCT a.name,MD5(a.name) FROM test_matrix.atributs a 
		LEFT JOIN test_matrix.atributs_uniq au ON a.hash =au.hash 
		WHERE au.id is NULL;
	-- Добавим новые атрибуты в atributs_uniq
	INSERT INTO test_matrix.atributs_uniq (name,hash )
		SELECT name,hash FROM test_matrix.lost_atr ;
